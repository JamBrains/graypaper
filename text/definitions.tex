...
\begin{description}
  ...
  \item[$\valcount$] The set of $\mathbb{V}$alidator key-set sizes. See equation \ref{eq:valcount}.
...
%  \item[$\Pi$] Unused.
  ...
  \item[$\computereport$] The work-report computation function. See equation \ref{eq:computereport}.
  \item[$\makebundle$] The bundle assembly function. See equation \ref{eq:makebundle}.
...
\begin{description}
  ...
  \item[$\erasurecode{v}{n}{\dots}$] The erasure-coding function for $v$ validators and $n$ pieces. See equation \ref{eq:erasurecoding}.
  \item[$\fnecoriginalshards(\dots)$] The original-chunk-count function. See equation \ref{eq:ecoriginalshards}.
  ...
  \item[$\fnmerklejustsubpath{x}(\dots)$] The justification path to a specific $2^x$ size page of a constant-depth Merkle tree. See equation \ref{eq:constantdepthsubtreemerklejust}.
  \item[$\keys{\dots}$] The domain, or set of keys, of a dictionary. See section \ref{sec:dictionaries}.
  \item[$\fnmerklesubtreepage{x}(\dots)$] The $2^x$ size page function for a constant-depth Merkle tree. See equation \ref{eq:constantdepthsubtreemerkleleafpage}.
  ...
  \item[$\ecrecover{v}{n}{\dots}$] The erasure-coding recovery function for $v$ validators and $n$ pieces. See equation \ref{eq:erasurecodinginv}.
...
\begin{description}
  ...
  \item[$\guarantorassignments$] The current core assignments. See section \ref{sec:coresandvalidators}.
  \item[$\guarantorassignmentsunderlastrotation$] The core assignments in the previous rotation. See section \ref{sec:coresandvalidators}.
...
\begin{description}
  ...
  \item[$\availassignments$] The availability assignments. A core's availability assignment is the work-report guarantee which is being made available prior to accumulation. See equation \ref{eq:reportingstate}.
  \begin{description}
...
\begin{description}
  ...
  \item[$\Cassurancetimeoutperiod = 5$] The period in timeslots after which reported but unavailable work is cleared. See equation \ref{eq:availassignmentspostassurancesdef}.
  ...
  \item[$\Csegmentsize = 4104$] The size of a segment in octets. See section \ref{sec:segments}.
  \item[$\Csegmentfootprint = \Csegmentsize + 32\ceil{\log_2(\Cmaxpackageexports)} = 4488$] The additional footprint in the Audits DA of a single imported segment. See equation \ref{eq:segmentfootprint}.
